...
class Killaura(timerManager: TimerManager): Module("Killaura"), IRenderable
{
    private val limiter = AttackRangeLimiter(maxReach = 3.0)
    ...
    private fun rayTraceCheck(target: Entity): Boolean {
        ...
        val attackerEyePosition = EntityPositions.head(player)
        ...
        for(sample in targetSamples) {
            ...
            if(RayTrace.trace(player, dir, limiter.maxReach, blockReach, world) == target)
                return true
        }
    ...
    private fun simulateAttack(target: Entity) {
        ...
        MinecraftForge.EVENT_BUS.post(AttackEntityEvent(player, target))
    }
